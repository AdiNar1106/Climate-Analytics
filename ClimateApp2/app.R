...
ui <- dashboardPage(
    dashboardHeader(title = 'CLIMATE.DF'),
    dashboardSidebar(width = 100
    ),
    dashboardBody(
        
        tags$h3('Climate Change across the world', style="text-align:center;color:#b30000;font-size:200%"),
        br(),
        fluidRow(
...
load("Datasets/cru_1901_2013_tmp_coarser")
...
server <- function(input, output) {
    ...
    output$distPlot <- renderPlot({
        ...
        mapplot=ggplot() +  mapWorld +ggtitle("Double click over any land part to see the temperature trend since 1901")+
    ...
    output$dbclick<-renderPlot({
        ...
        if(!is.na(lon_index())){
            ...
            if(length(na.omit(monthly))>0){
                ...
                temp_plot=ggplot(annual,aes(x=years,y=annual_ave))+geom_line()+ ylab(expression("Temperature "*~degree*C))+theme_bw() +ggtitle(paste0("Annual Average Temperature Trend at (",lon[lon_index()],", ", lat[lat_index()],")"))+xlab('')+theme(axis.title.y = element_text(size=14,angle=90,hjust=.5,vjust=1),
